...
let [embeddingResult, googleSheetQuery] = await Promise.all([
  lib.openai.playground['@0.0.5'].embeddings.create({
...
if (category > 0) {
  ...
  if (cachedRows.length === googleSheetQuery.rows.length) {
    ...
    while (inputs.length) {
      ...
      let embeddingResult = await lib.openai.playground['@0.0.5'].embeddings.create({
        model: `text-embedding-ada-002`,
        input: batchedInputs
      });
  ...
  let completion = await lib.openai.playground['@0.0.5'].completions.create({
